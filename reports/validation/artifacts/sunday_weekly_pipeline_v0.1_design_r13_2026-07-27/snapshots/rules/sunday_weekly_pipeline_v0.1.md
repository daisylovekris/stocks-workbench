...
| `blocked_identity_conflict` | `blocked` / `identity_mismatch`、`legacy_review_sha_unverifiable`、`phase_b_evidence_missing`、`phase_b_evidence_invalid`、`phase_c_evidence_missing`、`phase_c_evidence_invalid`、`ambiguous_phase_c_authority`、`phase_c_state_invalid`、`phase_c_permission_invalid`、`candidate_schema_identity_invalid`、`candidate_semantics_identity_invalid`、`input_set_schema_identity_invalid`、`input_set_contract_identity_invalid`、`unsupported_symbol`、`unresolved_value_encoding_invalid`、`ambiguous_orphan_completion` 或 `forbidden_automatic_trading_field` | 全 false | 是 | Phase C index path/entry inspection、expected/actual manifest path、raw SHA、schema/identity finding、available_inputs/missing_inputs、request key、Phase B/Phase C resolver evidence 或 orphan inventory |
| `already_completed` | `valid_matching_completion` 或 `index_recovered` | 同原 candidate；交易等仍 false | 是 | 现场重验的完整 prior manifest、current input set SHA、index correlation |
| `no_candidate_for_window` | `no_trading_days` | 全 false | 是 | calendar coverage、window dates、diagnostic；不得创建 candidate/index |
