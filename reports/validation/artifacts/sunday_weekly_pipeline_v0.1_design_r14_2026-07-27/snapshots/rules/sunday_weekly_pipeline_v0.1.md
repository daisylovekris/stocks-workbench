...
10. candidate schema canonical path 为 `rules/sunday_weekly_candidate_v0.1.schema.json`，candidate semantics canonical path 为 `rules/sunday_weekly_candidate_semantics_v0.1.md`，input-set schema canonical path 为 `rules/sunday_weekly_input_set_v0.1.schema.json`，input-set contract canonical path 为 `rules/sunday_weekly_input_set_v0.1.md`。初始、最终、valid matching completion/no-op 与 orphan recovery 均安全同 FD 读取四者并取得各自 bytes/SHA/parse，与 semantic config 的四枚 raw SHA 比较；schema/semantics/input-set schema/input-set contract 漂移依次为 `candidate_schema_identity_invalid`、`candidate_semantics_identity_invalid`、`input_set_schema_identity_invalid`、`input_set_contract_identity_invalid`，均属于 `blocked_identity_conflict`、全 false、可重跑。completion/no-op/index recovery 还须由当前输入重建 candidate canonical UTF-8 bytes（`sort_keys=true`、`separators=(',', ':')`）并与原 bytes 严格相等。review locks 只能由 `review_manifest.review_lock_path(runtime_dir, symbol, trade_date)` 取得，禁止复制 hash 算法；所有返回路径排序后统一获取。所有路径 token 均为 SWP-owned runtime dynamic tokens；外部 Phase B/C 路径按其 canonical contract 校验。
...
| 门 | 现场验证 | 失败状态 |
|---|---|---|
| G1 calendar | 涉及的全部年度 calendar 均存在、真实 schema 合法；窗口每个自然日落入恰好一份 coverage，并仅由 coverage + `trading_days` 分类；coverage/window 合法后计算 `trading_dates`，所有参与 raw SHA 入 input set。`trading_dates=0` 时直接 `no_candidate_for_window` / `no_candidate` / `no_trading_days` / all false，只写诊断、无 candidate、无 completion index；`trading_dates>=1` 才进入 G2—G8。 | coverage/window 不合法才为 `blocked_invalid_facts` / `blocked` / `calendar_window_invalid` / all false |
...
`swp_semantic_config_sha256` 是唯一权威文件 `rules/sunday_weekly_pipeline_semantic_config_v0.1.json` 的 raw canonical bytes SHA-256；不得从规则文字或运行参数重新拼装。它必须覆盖 candidate schema/semantics/input-set schema/input-set contract raw SHA、`daily_review_resolvers`、partial whitelist、accepted Phase C states、time registry、timezone、cutoff、provenance 与 rebuild policy。仅在全部输入完整后，严格按 input-set schema 与 contract 生成 input_set_sha256：只含 calendar identity、semantic config SHA，以及按 trade_date 升序的 facts SHA、由已登记 resolver 精确定位的真正 Daily Review SHA、Phase B manifest SHA、authoritative Phase C manifest SHA。未登记 symbol 为 `blocked_identity_conflict:unsupported_symbol`；Phase C index bytes、bundle/snapshot 路径、diagnostics、candidate bytes、validator output 均不得进入 input set，仅可写运行 manifest 并在最终重解。裸 JSON number unresolved value 为 `blocked_identity_conflict:unresolved_value_encoding_invalid`，不得字符串化。窗口零交易日为 `no_candidate_for_window:no_trading_days`，全 false，仅诊断，不建 candidate 或 completion index。Phase B `semantic_noop` 仅是输入证据；SWP no-op 唯一 reason 为 `valid_matching_completion`。`logical_run_key=sha256(canonical JSON {request_key,input_set_sha256})`；候选生成后 `semantic_completion_key=sha256(canonical JSON {logical_run_key,input_set_sha256,candidate_sha256})`。
...
| state | outcome | reason_code | permissions |
|---|---|---|---|
| `candidate_ready_for_human_review` | `candidate_created` | `all_gates_passed` | human_review only |
| `blocked_missing_facts` | `blocked` | `facts_missing:<date>` | all false |
| `blocked_invalid_facts` | `blocked` | `facts_validator_failed` | all false |
| `blocked_invalid_facts` | `blocked` | `facts_sha_drift` | all false |
| `blocked_invalid_facts` | `blocked` | `calendar_window_invalid` | all false |
| `blocked_invalid_facts` | `blocked` | `as_of_date_invalid` | all false |
| `blocked_invalid_facts` | `blocked` | `metrics_field_missing_or_invalid` | all false |
| `blocked_missing_daily_review` | `blocked` | `daily_review_missing:<date>` | all false |
| `blocked_future_data` | `blocked` | `future_input_detected` | all false |
| `blocked_future_data` | `blocked` | `source_time_unverifiable` | all false |
| `blocked_identity_conflict` | `blocked` | `identity_mismatch` | all false |
| `blocked_identity_conflict` | `blocked` | `legacy_review_sha_unverifiable` | all false |
| `blocked_identity_conflict` | `blocked` | `phase_b_evidence_missing` | all false |
| `blocked_identity_conflict` | `blocked` | `phase_b_evidence_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `phase_c_evidence_missing` | all false |
| `blocked_identity_conflict` | `blocked` | `phase_c_evidence_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `ambiguous_phase_c_authority` | all false |
| `blocked_identity_conflict` | `blocked` | `phase_c_state_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `phase_c_permission_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `candidate_schema_identity_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `candidate_semantics_identity_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `input_set_schema_identity_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `input_set_contract_identity_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `unsupported_symbol` | all false |
| `blocked_identity_conflict` | `blocked` | `unresolved_value_encoding_invalid` | all false |
| `blocked_identity_conflict` | `blocked` | `ambiguous_orphan_completion` | all false |
| `blocked_identity_conflict` | `blocked` | `forbidden_automatic_trading_field` | all false |
...
| `no_candidate_for_window` | `no_candidate` | `no_trading_days` | all false |
| `failed` | `failed` | `runtime_io_error` | all false |
| `failed` | `failed` | `lock_error` | all false |
| `failed` | `failed` | `unexpected_exception` | all false |
