...
5. 固定市场事实判断截止为最后一个交易日 `Asia/Shanghai` 收盘。`source_kind` 是 SWP 从已登记字段路径**内部标准化**出的类型，不要求输入 JSON 有同名字段；enum 仅为 `daily_bar`、`announcement`、`news`、`policy`、`provenance`。当前 `facts_pack_v0.2` 的登记映射为：`generated_at` 与任意 `**.fetched_at` → provenance；`trade_date`、`run.target_date`、任意 `**.source_date` 与 `volume_ratio.*.trade_dates[]` → daily_bar。公告、新闻、政策只能由各自显式登记的结构化字段映射。`daily_bar` 仅接受 `YYYY-MM-DD`；`announcement`、`news`、`policy` 仅接受含数值 UTC offset 或 `Z` 的 RFC3339 datetime，naive datetime 一律拒绝，外部市场事件的 date-only 一律拒绝。`provenance`（采集、生成、hash、runtime 写入）独立于市场事实，不参与 cutoff，可晚于 cutoff。自由文本日期永不构成机器时间证据；未登记时间字段、未知 schema version、路径语义冲突，或外部事实只有自由文本而无合格结构化时间，均为 `blocked_future_data:source_time_unverifiable`（fail-closed），不得以 `needs_manual_check` 放行。合格市场事实转换为 Asia/Shanghai 后，晚于 cutoff 为 `blocked_future_data:future_input_detected`。
6. 原样保留 `missing`、`needs_manual_check`、TODO/null 背景缺口；不补齐、不将缺口解释为利多/利空。
7. 每个交易日的 Phase B canonical evidence path 固定为 `<runtime>/phase-b/<symbol_sha256>/<trade_date>/runner_manifest.json`，其 schema 必须为 `runner_manifest_v0.2_phase_b`，并须经 Phase B completion live validator 验证 symbol、target_date、official identity、write semantics 和 committed evidence。每个交易日的 Phase C canonical evidence path 固定为 `<runtime>/phase-c/<symbol_sha256>/<trade_date>/review_manifest.json`，其 schema 必须为 `review_manifest_v0.2_phase_c`，并须验证 identity、runner reference、权限与 review_state。两者均以安全读取的 raw bytes SHA-256 作为现场身份；不存在、非 canonical path、schema 或身份无效都不得用另一阶段 evidence 推断或替代。
8. 生成 immutable candidate、manifest、summary 和验证证据，全部在仓库外受控 runtime；正式 weekly 必须由人工另行写入。
...
| G4 integrity | 每个交易日 Phase B/Phase C canonical evidence 存在、schema/identity 合法，安全读取 raw SHA；全部 SHA、semantic config SHA 与本次 manifest/input set 一致；最终复读未漂移 | `blocked_identity_conflict` |
...
`swp_semantic_config_sha256` 是固定 semantic config 的 canonical JSON SHA-256；该 JSON 必须完整覆盖 candidate schema version、partial whitelist、accepted Phase C state whitelist、time evidence path registry、`Asia/Shanghai` timezone/cutoff contract。仅在全部 input 完整后生成 `input_set_sha256`：它是所有参与分类 calendar（按 year/path 升序）的 raw SHA、按交易日排序的 facts raw SHA、Daily Review raw SHA、每日 Phase B evidence raw SHA、每日 Phase C evidence raw SHA、`swp_semantic_config_sha256` 的 canonical JSON 哈希。`logical_run_key=sha256(canonical JSON {request_key,input_set_sha256})`；候选生成后 `semantic_completion_key=sha256(canonical JSON {logical_run_key,input_set_sha256,candidate_sha256})`。manifest 必须写入 semantic config SHA 及逐日两阶段 evidence canonical path/schema/raw SHA；completion 现场复验必须重算并比较它们。任何 Phase B evidence 或 semantic config 变化都必须产生新的 input set。每次 completion 分配随机 128-bit 或等价不可预测、非输入导出的 `completion_attempt_id`；每次 blocked/failed 分配同性质且独立的 `diagnostic_attempt_id`。物理 completion identity 为 `{semantic_completion_key, completion_attempt_id}`，而不是固定目录名。SWP no-op 只承认有 index 精确关联且全量现场复验通过的相同 `semantic_completion_key`；Phase B semantic evidence 仅是 G4/G7 输入证据，绝不外溢为 SWP completion。candidate canonical digest 只含 schema 固定字段、输入身份、确定性 metrics、unresolved fields、固定人工槽位；不得含生成时间、run/attempt id、主机、pid、路径、锁等待或 diagnostics 等易变 metadata。候选 SHA 为该 canonical candidate JSON 的 SHA-256。所有 runtime 路径 token 只能使用固定字面量和 `[0-9a-f]` SHA/attempt token；解析出的 symbol、日期或原始参数不得写入路径。
...
sunday_weekly_request_<request_key>.lock
  ...
  → 提交 index，释放 facts locks，再释放 request-key orchestration lock
```
...
| `blocked_identity_conflict` | `blocked` / `identity_mismatch`、`legacy_review_sha_unverifiable`、`phase_b_evidence_missing`、`phase_b_evidence_invalid`、`phase_c_evidence_missing`、`phase_c_evidence_invalid`、`phase_c_state_invalid`、`phase_c_permission_invalid`、`ambiguous_orphan_completion` 或 `forbidden_automatic_trading_field` | 全 false | 是 | expected canonical path、actual path、raw SHA、schema/identity finding、available_inputs/missing_inputs、request key、review/Phase B/Phase C evidence 或 orphan inventory |
