...
7. Phase C authority 只能由 `<runtime>/reviews/<trade_date>/review_index.jsonl` 解析，不得根据 symbol hash 猜 manifest 路径。安全读取 index 后逐项调用现行 `review_manifest.inspect_index_entry`，核验 review_id、canonical manifest_path、manifest SHA；仅保留 `symbol`、`trade_date`、current official SHA 均匹配且 review_state 精确属于 semantic config 两项白名单的条目，并确认所有 downstream permissions 仍为 false。恰好一项 authoritative valid Phase C entry 才可继续；零项合法 entry 为 `blocked_identity_conflict:phase_c_evidence_missing`，存在非法候选但无合法 entry 为 `blocked_identity_conflict:phase_c_evidence_invalid`，两个或更多相互冲突的合法 authority 为 `blocked_identity_conflict:ambiguous_phase_c_authority`。Phase C raw SHA 是该唯一 manifest 的安全读取 bytes SHA。
